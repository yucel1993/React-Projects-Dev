...
// ! Uppercode there is no authorization control  but in ht ebellow code there is a authorization control
...
const BlogDetail = ({ setInfo: setUpperInfo }) => {
  ...
  const handleDelete = () => {
    // Check if the logged-in user's username matches the author's username
    if (username === author) {
      deleteBlog(id);
      navigate(-1);
      setShowModal(false);
    } else {
      // Show an unauthorized user message if the user is not the author
      alert("Unauthorized user: You can't delete this blog.");
    }
  };
  ...
  return (
    <div style={{ marginTop: "2rem", marginBottom: "2rem" }}>
      <Box sx={{ width: "50vh", margin: "auto" }}>
        ...
        <Box sx={{ display: "flex", justifyContent: "center" }}>
          {username === author && ( // Render these buttons only if the user is the author
            <>
              <Button
                variant="contained"
                onClick={() => {
